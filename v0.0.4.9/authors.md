# Authors and Citation

## Authors

- **Isaac Gravestock**. Author, maintainer.
  [](https://orcid.org/0000-0003-0283-2065)

- **Li Su**. Author.

- **Roonak Rezvani**. Author.
  [](https://orcid.org/0000-0001-5580-5058)  
  Original package author

- **Julia Moesch**. Author.

- **Medical Research Council (MRC)**. Funder.

- **F. Hoffmann-La Roche AG**. Copyright holder, funder.

## Citation

Source:
[`DESCRIPTION`](https://github.com/Causal-LDA/TrialEmulation/blob/v0.0.4.9/DESCRIPTION)

Gravestock I, Su L, Rezvani R, Moesch J (2026). *TrialEmulation: Causal
Analysis of Observational Time-to-Event Data*. R package version
0.0.4.9, <https://causal-lda.github.io/TrialEmulation/>.

    @Manual{,
      title = {TrialEmulation: Causal Analysis of Observational Time-to-Event Data},
      author = {Isaac Gravestock and Li Su and Roonak Rezvani and Julia Moesch},
      year = {2026},
      note = {R package version 0.0.4.9},
      url = {https://causal-lda.github.io/TrialEmulation/},
    }
