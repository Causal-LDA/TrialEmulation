...
#'
#' Apply a weighted pooled logistic regression to fit the marginal structural model for the sequence of emulated trials and
#' calculates the covariance matrix  of parameter using the robust sandwich estimator.
#'
#' @param use_sample_weights Use case-control sampling weights in addition to inverse probability weights for treatment and censoring. 
#' `data` must contain a column `sample_weight`. The final weights used in the pooled logistic regression are calculated as `weight = weight * sample_weight`.
...
#'  \item{robust}{a list containing a summary table of regression coefficient estimates and the robust covariance `matrix`}
