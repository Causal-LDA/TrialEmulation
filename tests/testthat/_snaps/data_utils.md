# weight_func works as expected
    ...
    Code
      ...
      cense_n_cov = ~ X3 + X4, save_weight_models = "tidy", save_dir = save_dir)
    Output
...
# weight_func works saves model objects

    Code
      result <- weight_func(sw_data = data, switch_n_cov = ~1, switch_d_cov = ~ X1 +
        X2, cense = "C", pool_cense = 0, cense_d_cov = ~ X1 + X2 + X3 + X4 + age_s,
      cense_n_cov = ~ X3 + X4, save_weight_models = "object", save_dir = save_dir)
    Output
      [1] "P(treatment=1 | treatment=0) for denominator"
