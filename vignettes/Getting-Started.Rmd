...
[^csv]: Saving the data to disk as csv allows us to use various techniques when the expanded data set becomes very 
large and no longer fit easily in memory.
...
Instead of using `initiators()` it is possible to run each step individually with `data_preparation()`,
`data_modelling()` etc.
