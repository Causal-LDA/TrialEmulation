...
We can access the summaries of the weight models to inspect how the IP weights have been calculated. 
```{r}
load(file.path(working_dir, "weight_models.rda"))
...
Here we use the patients who were in the first trial as the target population.

```{r}
new_data <- data.table::fread(file.path(working_dir, "trial_1.csv"), colClasses = prep_data$col_classes)
