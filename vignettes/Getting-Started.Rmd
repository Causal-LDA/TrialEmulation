...
model_preds <- predict_cum_inc(model = model_result$model, predict_times = c(0:40), newdata = new_data)
```

We can easily compare the curves visually.

```{r plot_cuminc}
plot(c(0:40), model_preds$trt_0, ty = "l", ylab = "Cumulative Incidence", xlab = "Follow-up Time")
lines(c(0:40), model_preds$trt_1, col = 2)
```

```{r plot_cuminc_diff}
ci_differences <- model_preds$trt_1 - model_preds$trt_0
plot(c(0:40), ci_differences, ty = "l", ylab = "Cumulative Incidence Difference", xlab = "Follow-up Time")
```


```{r}
model_preds_new <- predict(model_result, predict_times = c(0:40), newdata = new_data)
plot(
  model_preds_new$difference$followup_time,
  model_preds_new$difference$estimate,
  ty = "l", ylab = "Cumulative Incidence Difference",
  xlab = "Follow-up Time"
)
lines(model_preds_new$difference$followup_time, model_preds_new$difference$`2.5%`, lty = 2)
lines(model_preds_new$difference$followup_time, model_preds_new$difference$`97.5%`, lty = 2)
```
