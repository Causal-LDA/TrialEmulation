...
It is also possible to specify formulas in `outcome_modelling` to include, for example, splines with
`ns()`.

Now we can fit the model with the `outcome_modelling()` function. Since we have sampled from the data, 
we should make use of the `use_sample_weights` option to get correct survival estimates.

```{r modelling}
model_result <- outcome_modelling(
  data = sampled_data,
  outcome_cov = c("catvarA", "catvarB", "nvarA", "nvarB", "nvarC"),
  model_var = "assigned_treatment",
  use_weight = TRUE,
