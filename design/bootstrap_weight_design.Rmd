...
Currently, predict.R is the script that specifies the function predict() above. If the command 'type = "survival" is inputed, the resulting difference in `preds$difference` is an estimate for the estimand  $\Pr(Y_{m,k}^{\overline a_k= \overline 1} = 0) - \Pr(Y_{m,k}^{\overline a_k= \overline 0} = 0)$. I propose we have a new 'type = 'mrd'' so that we are estimating the marginal risk difference, $\Pr(Y_{m,k}^{\overline a_k= \overline 0} = 0) - \Pr(Y_{m,k}^{\overline a_k= \overline 1} = 0)$ (which is just the opposite).

Modify line 63 of predict.R to: 

```{r}
pred_fun <- if (type %in% c("survival", 'mrd')) {
    calculate_survival
...
```{r}
if (type == 'mrd') {
  pred_list$difference <- pred_list$assigned_treatment_0 - pred_list$assigned_treatment_1
} else pred_list$difference <- pred_list$assigned_treatment_1 - pred_list$assigned_treatment_0
```
...
```{r}
 pred_list <- calculate_predictions(
  ...
  if (type == 'mrd'){
    bootstrap_CIs <- function_that_calculates_bootstrap_CIs(output, ci_type, MSM_model_object )
  }
```
