...
tte <- target_trial_sequence() |> 
  set_data(my_big_df, id = "ID", treatment = "drug", outcome = "Death", eligibility = "IE") |> 
  set_estimand("PP")
# message:
# A per-protocol analysis requires specification of treatment switching model with set_switch_weight_model()
tte2 <- tte |> 
  set_switch_weight_model(numerator = ~ age + biomarker, denominator = ~ age + biomarker, pooled = TRUE, save_models = "~/my_TTE") |> 
